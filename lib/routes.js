...
module.exports = (reporter, definition, app) => {
  ...
  app.use('/w/:user/:shortid*', (req, res, next) => {
    ...
    // this is workaround to avoid unnecessary redirects which could be bad for SEO
...
function addRequestStartEndLog (app) {
  ...
  app.use(morgan(':id :method :url w=:workspace d=:date[iso] START', {
    immediate: true,
    skip: morganSkip,
    stream: process.stdout
  }))

  app.use(morgan(':id :method :url w=:workspace d=:date[iso] status=:status time=:response-time ms END', {
    skip: morganSkip,
    stream: process.stdout
  }))
}
