...
module.exports = function (reporter, definition) {
  ...
  reporter.initializeListeners.add('playground', async () => {
    if (reporter.options.store.provider === 'mongodb') {
      const textIndexes = {name: 'text', description: 'text'}

      reporter.logger.debug(`Creating text indexes for ${Object.keys(textIndexes).join(', ')} field(s) in workspaces collection`)
      await reporter.documentStore.provider.db.collection('workspaces').createIndex(textIndexes)
    }
    ...
    for (const name in reporter.documentStore.collections) {
      ...
      col.beforeFindListeners.add('playground', col, (query, select, req) => {
        ...
        if (col.name === 'workspaces') {
          throw reporter.createError('Workspaces queries not supported')
        }

        if (col.name === 'users') {
          throw reporter.createError('Users queries not supported')
        }
