...
module.exports = (reporter) => {
  return {
    ...
    async loadWorkspace (workspace, user, userId) {
      ...
      if (user) {
        workspace.hasLike = (await reporter.documentStore.collection('likes').findOne({ userId: user._id, workspaceId: workspace._id })) != null
      }
    ...
    async removeWorkspace (workspaceId, userId) {
      const w = await reporter.documentStore.collection('workspaces').findOne({
        _id: workspaceId
      })
      ...
      await reporter.documentStore.collection('likes').remove({
        workspaceId
      })

      await reporter.documentStore.collection('workspaces').remove({
        _id: workspaceId
      })
    },
    ...
    async searchWorkspaces (search) {
      ...
      const workspaces = await reporter.documentStore.collection('workspaces').find(...searchParams)
      ...
      return {
        items: workspaces,
        count: await reporter.documentStore.collection('workspaces').find(...searchParams).count(0)
      }
    },
