...
module.exports = (reporter) => {
  return {
      ...
      if (!currentWorkspace.__isInitial) {
        if (currentWorkspace.userId !== userId) {
          ...
          for (const name in reporter.documentStore.collections) {
            if (name === 'settings' || name === 'workspaces' || name === 'profiles') {
              continue
            }
            ...
            for (const e of entities) {
              ...
              const entityWithConvertedBuffers = (await col.deserializeProperties([entityToInsert]))[0]
    ...
    async deleteUser (userId) {
      ...
      if (!user) {
        throw reporter.createError('user does not exists', {
          statusCode: 400,
          weak: true
        })
      }
      ...
      reporter.logger.debug(`Removing user "${user.username}" with _id ${userId} and all its workspaces`)

      await reporter.documentStore.internalCollection('users').remove({
        _id: userId
      })
    },
